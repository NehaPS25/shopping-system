...
try{
	...
	ResultSet rs = st.executeQuery("SELECT * FROM cart INNER JOIN Productss ON cart.product_id = Productss.id WHERE cart.orderDate IS NOT NULL AND cart.status = 'processing'");
	while(rs.next())
	{
...
<td>
    <a href="cancelOrdersAction.jsp?id=<%=rs.getString(2)%>&email=<%=rs.getString(1)%>">
        Cancel <i class='fas fa-window-close'></i>
                ...
                <td><a href="deliveredReceivedOrdersAction.jsp?id=<%=rs.getString(2)%>&email=<%=rs.getString(1)%>">Delivered <i class='fas fa-dolly'></i></i></a></td>
            </tr>
         <%
	}
	}catch(Exception e)
